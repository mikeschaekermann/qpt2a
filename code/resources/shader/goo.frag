...
void main(void) {
	...
	//vec3 uvw = 0.1 * vec3(cnoise(vec3(vertexPositionModel.x * t, 0., 0.)),
	//	cnoise(vec3(0., vertexPositionModel.y * t, 0.)),
	//	cnoise(vec3(0., 0.,  vertexPositionModel.z * t)));

	vec2 uv = vertexPositionModel.xy + 0.1 * vec2(cnoise(vertexPositionModel.xy + vec2(t, 0.0)),
		cnoise(vertexPositionModel.xy + vec2(43.0, t)));
	
	float n = cnoise(uv - vec2(0., t));
	...
	gl_FragColor = diffuseColor; // ambientColor + diffColor + specColor;
}
